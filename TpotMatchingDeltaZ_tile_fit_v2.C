...
using z_range_t = std::pair<float,float>;
using z_range_vector_t = std::vector<z_range_t>;
z_range_vector_t tiles_z_range = {
  { -85, -65 },
  { -50, -5 },
  { 5, 50 },
  { 65, 105 },
...
double fit_function( double* x, double* par )
{
  ...
  if( z<tiles_z_range[itile].first || z>tiles_z_range[itile].second )
  { TF1::RejectPoint(); }
...
//_______________________________________________________________
void TpotMatchingDeltaZ_tile_fit_v2()
{

  set_style( false );
  save_detector_names();
  const TString tag = "_CombinedDataReconstruction_corrected_benjamin-new";
  // const TString tag = "_CombinedDataReconstruction_bade";
  ...
  {
    // const TString inputFile = Form( "/gpfs/mnt/gpfs02/sphenix/user/bade/sphenix/work/g4eval/DST_TPC/newfiles_corrected_24022025/dst_eval-%08i-*-full.root", runnumber );
  ...
  static constexpr int nzbins = 100;

  // 3D histogram
  TString hname = "h_dz";
  auto h = new TH3F( hname, "", 8, 0, 8, nzbins, -max_z, max_z, 100, -max_dz, max_dz);
  // Utils::max_entries = 50000;
  ...
  auto h_fit = new TH2F( hname, "", 8, 0, 8, nzbins, -max_z, max_z);
  h_fit->GetXaxis()->SetTitle( "tile number" );
  h_fit->GetYaxis()->SetTitle( "z (track) (cm)" );
  h_fit->GetZaxis()->SetTitle( "#Deltaz(track-cluster) (cm)" );
  h_fit->SetStats(0);
  rootFile.Add(h_fit);
  ...
  for( int itile = 0; itile < 8; ++itile )
  {
    ...
    // vertical lines
    Draw::VerticalLine( gPad, tiles_z_range[itile].first )->Draw();
    Draw::VerticalLine( gPad, tiles_z_range[itile].second )->Draw();
