...
void RawDataNoiseEvaluation(int runNumber = 45955)
{
  ...
  const TString pdfFile = Form( "Figures/RawDataNoiseEvaluation-%08i-0000.pdf", runNumber );
  const TString outputFile = Form( "Rootfiles/RawDataNoiseEvaluation-%08i-0000.root", runNumber );

  RootFile rootfile( outputFile );

  std::cout << "RawDataNoiseEvaluation - inputFile: " << inputFile << std::endl;
  std::cout << "RawDataNoiseEvaluation - pdfFile: " << pdfFile << std::endl;
  ...
  // auto container = new MicromegasCombinedDataEvaluation::Container;
  ...
  // auto h_adc_channel = new TH2I( "h_adc_channel", "", 4096, 0, 4096, 1024, 0, 1024 );
  ...
  // auto h_adc_channel_3d = new TH3F( "h_adc_channel_3d", "h_adc_channel_3d", 16, 0, 16, 256, 0, 256, 1024, 0, 1024 );
  ...
  std::cout << "RawDataNoiseEvaluation - entries: " << entries << std::endl;
  for( int i = 0; i < entries; ++i )
  {
    // some printout
    if( !(i%100) )
    { std::cout << "RawDataNoiseEvaluation - entry: " << i << std::endl; }
  ...
  rootfile.Add( h_adc_channel );
  rootfile.Add( p_adc_channel );
  rootfile.Add( h_adc_channel_3d );
  ...
  rootfile.Add( h_adc_channel_corrected );
  rootfile.Add( h_adc_corrected );
  ...
  if( true )
  {
    ...
    pdfDocument.Add(cv);
    // cv->SaveAs(Form( "Figures/RawDataNoiseEvaluation-%08i-0000_profile.png", runNumber ) );
    cv->SaveAs(Form( "Figures/RawDataNoiseEvaluation-%08i-0000_profile.pdf", runNumber ) );
  }
  ...
  if( true )
  {
   ...
   //  gPad->SetTopMargin( 0.07 );
    ...
    pdfDocument.Add(cv);
    // cv->SaveAs(Form( "Figures/RawDataNoiseEvaluation-%08i-0000_rms.png", runNumber ) );
    cv->SaveAs(Form( "Figures/RawDataNoiseEvaluation-%08i-0000_rms.pdf", runNumber ) );
