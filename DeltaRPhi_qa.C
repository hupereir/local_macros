...
TString DeltaRPhi_qa( TString tag = TString() )
{
  ...
  const TString pdfFile = Form( "Figures/DeltaRPhi_qa%s.pdf", tag.Data() );
  const TString rootFile  = Form( "Rootfiles/DeltaRPhi_qa%s.root", tag.Data() );
  ...
  auto h2d = static_cast<TH2*>(fileManager.GetHistogram( "h_QAG4SimulationDistortions_deltarphi_layer" ));
