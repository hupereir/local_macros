...
namespace
{
  ...
  void write_to_json( const TString& filename )
  {
    ...
    { 
      ...
      {
        std::cout << "write_to_json - skipping mvtx cluster" << std::endl;
        continue;
      }
...
//____________________________________________________________________________
TString MVTX_EventDisplay_3dfit( int runnumber = 25475 )
{
  ...
  auto mvtx_tree = new TChain( "ClusterTree" );  
  mvtx_tree->Add(  Form( "/sphenix/user/hjheng/macros/detectors/sPHENIX/OutFile/ClusterTree/ClusterTree_Run%i_0.root", runnumber ) );
  mvtx_tree->Add(  Form( "/sphenix/user/hjheng/macros/detectors/sPHENIX/OutFile/ClusterTree/ClusterTree_Run%i_1.root", runnumber ) );
  mvtx_tree->Add(  Form( "/sphenix/user/hjheng/macros/detectors/sPHENIX/OutFile/ClusterTree/ClusterTree_Run%i_2.root", runnumber ) );
  ...
  for(; intt_entry < intt_entries && mvtx_entry < mvtx_entries && tpot_entry < tpot_entries; ++intt_entry )
  {
    ...
    // perform circle fit
    const auto [r,x0,y0] = circle_fit_by_taubin( fit_positions_xy );
    ...
    if( r > 0 )
    {
      ...
      static constexpr double bz_const = 1.4; // Tesla
      pt = r*0.3*bz_const/1e2; // GeV
      
      std::cout << "MVTX_EventDisplay_3dfit - pT: " << pt << " GeV/c" << std::endl;
      
      const auto [xf,yf] = fit_positions_xy.front();
      const auto begin = std::atan2( yf-y0, xf-x0 ) * rad_to_deg;
      
      const auto [xb,yb] = fit_positions_xy.back();
      const auto end = std::atan2( yb-y0, xb-x0 ) * rad_to_deg;
      
      auto arc = new TArc( x0, y0, r, begin, end);
      arc->SetFillStyle(0);
      arc->SetLineColor(4);
      arc->Draw( "only" );
    }
    ...
    {
      // auto text = new TPaveText(0.66,0.68,0.94,0.89, "NDC" );
...
//       if( pt > 0 )
//       { text->AddText(Form( "#it{p}_{T} = %.1f GeV/#it{c}", pt ) ); }
    ...
    {
      const auto [rf,zf] = positions_rz.front();
      const auto zbegin = z0 + slope*rf;
      
      const auto [rb,zb] = positions_rz.back();
    ...
    // get track parameters at largest z point
    if( r > 0 )
    {
      
      const auto [xb,yb] = fit_positions_xy.back();
      const double phi0 = std::atan2( yb-y0, xb-x0 );

      const auto [xf,yf] = fit_positions_xy.front();
      const double phi1 = std::atan2( yf-y0, xf-x0 );
      
      track_parameters_t track;
      
      // get vertex position
      track.xv = x0 + r*std::cos( phi0 );
      track.yv = y0 + r*std::sin( phi0 );
      const double rv = get_r(track.xv, track.yv);
      track.zv = z0 + slope*rv; 
      
      // the signs have been adjusted manually to make the display look good. 
      // part of it should actually be absorbed in the track charge
      track.px = -pt*std::sin( phi0 );
      track.py = pt*std::cos( phi0 );
      track.pz = -pt*slope;
      ...
      track.length = r*std::abs(phi0-phi1)*1.02;
