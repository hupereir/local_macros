...
TString DeltaZ_cluster_new( TString tag = TString() )
{
  ...
  #define USE_TRACKS
  #ifdef USE_TRACKS
  const TString var( "m_tracks.clusters.z - m_tracks.clusters.truth_z" );
  const TString var2d = Form( "%s:m_tracks.clusters.layer", var.Data() );
  // const TCut cluster_cut( "_clusters.size == 1" );
  const TCut cluster_cut( "m_tracks.clusters.z > 0" );
  #else
  const TString var( "m_clusters.z - m_clusters.truth_z" );
  const TString var2d = Form( "%s:m_clusters.layer", var.Data() );
  // const TCut cluster_cut( "_clusters.size == 1" );
  const TCut cluster_cut( "m_clusters.z > 0" );
  #endif
