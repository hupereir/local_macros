...
TString DeltaZ_tpc()
{
  ...
  // input files
  const TString tag = "_truth_notpc_nodistortion-new";
  // const TString tag = "_truth_notpc_distorted_coarse";
  const TString inputFile = Form( "DST/CONDOR_realistic_micromegas/dst_reco%s/dst_reco_realistic_micromegas_*.root", tag.Data() );
  
  const TString pdfFile = Form( "Figures/DeltaZ_tpc%s.pdf", tag.Data() );
  ...
  for( int idet = 0; idet < nDetectors; ++idet )
  {
    ...
    for( int ilayer = 0; ilayer < nLayers[idet]; ++ilayer )
    {
      ...
      for( int icut = 0; icut < ncuts; ++icut )
      {
        ...
        std::cout << "DeltaZ_tpc - layer: " << layerIndex << " entries: " << entries << std::endl;
        if( entries )
        {
          if( do_fit )
          {
            ...
            if( result._valid )
            {
              ...
              std::cout << "DeltaZ_tpc - skipping layer " << layerIndex << " (failed fit)" << std::endl;
