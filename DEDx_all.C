...
TString dedx_all()
{
  ...
  const std::string global_tag = "_lpt-new";

  // pdf document
  const TString pdfFile = Form( "Figures/dedx_all%s.pdf", global_tag.c_str());
  PdfDocument pdfDocument( pdfFile );

  // create dummy histogram
  auto h_all = new TH2F( "dedx", "", 100, -5, 5, 100, 0, 1000 );
  h_all->GetXaxis()->SetTitle( "q.p [GeV]" );
  h_all->GetYaxis()->SetTitle( "dE/dx [ADC/cm]" );
  ...
  for( const auto& [pid, tag]:data_list )
  {
    ...
    const TString var( "_tracks._dedx:_tracks._p*_tracks._charge" );
