...
TString DeltaRPhi_Tony()
{
  ...
  if( false )
  {
    ...
    const TString inputFile = Form( "DST/CONDOR%s/TrackResiduals_*_?.root", tag.Data() );
    std::cout << "DeltaRPhi_Tony - inputFile: " << inputFile << std::endl;
    fileManager.AddFiles( inputFile );

  }

  const TString pdfFile = Form( "Figures/DeltaRPhi_Tony%s%s.pdf", tag.Data(), postfix.Data() );
  const TString rootFile  = Form( "Rootfiles/DeltaRPhi_Tony%s%s.root", tag.Data(), postfix.Data() );

  std::cout << "DeltaRPhi - pdfFile: " << pdfFile << std::endl;
  std::cout << "DeltaRPhi - rootFile: " << rootFile << std::endl;
  ...
  // mean vs layer
  {
...

    {
      ...
      const TString var = "fabs(clusgr)*delta_phi(atan2(stategy,stategx)-atan2(clusgy,clusgx))";
      const TString var2d = Form( "%s:cluslayer", var.Data() );
      const TCut layercut = "cluslayer<56";

      const auto hname = "h_drphi_2d";
      auto h = new TH2F( hname, "", 60, 0, 60, 100, -max_dphi, max_dphi );
      Utils::TreeToHisto( tree, hname, var2d, track_cut&&charge_cut&&layercut, false );
