...
//   std::array<int,n_intt_boards> intt_ids = {0, 1, 2, 3, 4, 5, 6, 7};

//   static constexpr int n_intt_boards = 4;
//   std::array<int,n_intt_boards> intt_ids = {1, 2, 5, 6};

  static constexpr int n_intt_boards = 4;
  std::array<int,n_intt_boards> intt_ids = {0, 3, 4, 7};
...
void INTT_Correlation_clusters_all(const int runnumber = 20446)
{
  set_style( false );
  // const std::string suffix = "_all";
  // const std::string suffix = "_up";
  const std::string suffix = "_down";
  
  const TString tpot_inputfilename = Form( "Rootfiles/RawDataClusterTree-%08i-0000-full.root", runnumber );
  const TString pdffilename = Form( "Figures/INTT_Correlation_clusters-%08i-0000%s.pdf", runnumber, suffix.c_str() );
  const TString pngfilename = Form( "Figures/INTT_Correlation_clusters-%08i-0000%s.png", runnumber, suffix.c_str() );
  const TString rootfilename = Form( "Rootfiles/INTT_Correlation_clusters-%08i-0000%s.root", runnumber, suffix.c_str() );
  ...
  {
    const TString intt_inputfilename = Form( "Rootfiles/intt_clusters/beam_intt%i-%08i-0000_clusters.root", intt_ids[i], runnumber );
  ...
  auto h_correlation = new TH2F( "h_correlation", "", 320, 0, 320, 320, 0, 1100*n_intt_boards );
  ...
  while( !empty )
  {
    ...
    if( !corrected ) 
    {
      ...
      for( int i = 0; i < n_intt_boards; ++i )
      {
        ...
        nclusters += intt_event_array[i]->fNclusters;
        assert(  intt_event_array[i] ==  intt_event_array[0] );
      }
      
      // fill correlation histogram
      h_correlation->Fill( tpot_container->n_clusters, nclusters );
...
void process_all()
{
  ...
  { INTT_Correlation_clusters_all( runnumber ); }
}
