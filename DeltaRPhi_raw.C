...
TString DeltaRPhi_raw()
{
  ...
  std::array<float, nDetectors> max_det_residual = { 0.1, 0.1, 2, 2, 2, 2, 2};
  ...
  const TString pdfFile = Form( "Figures/DeltaRPhi_raw%s%s.pdf", tag.Data(), postfix.Data() );
  const TString rootFile  = Form( "Rootfiles/DeltaRPhi_raw%s%s.root", tag.Data(), postfix.Data() );
