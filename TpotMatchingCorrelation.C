...
//   const TString inputFile = Form( "DST/CONDOR%s/dst_reco*.root", tag.Data());
  const TString tag = "_CombinedDataReconstruction-test";
  TString postfix;
  TString run_label;
  ...
  const TString rootFilename = Form( "Rootfiles/TpotMatchingCorrelation%s%s.root", tag.Data(),postfix.Data());
