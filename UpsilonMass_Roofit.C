...
void UpsilonMass_Roofit()
{
  ...
  const auto pdffilename = drawPulls ?
    Form( "Figures/UpsilonMass_Roofit%s_Pulls.pdf", tag.Data()):
    Form( "Figures/UpsilonMass_Roofit%s.pdf", tag.Data());
  ...
  std::cout << "UpsilonMass_Roofit - done with copy - entries: " << copy->GetEntries() << std::endl;
  ...
  RooRealVar  mean("mean", "mean", mass_upsilon_1s, 9, 10);
  RooRealVar  sigma("sigma", "sigma", 0.1, 0.0, 0.2);
  RooRealVar  width("width", "Lorentzian Width", 0.003, 0.001, 0.01);

  // left tail
  RooRealVar  cb_alpha_l("alphal", "alphal", 0.5, 0.1, 5);
  RooRealVar  cb_n_l("nl", "nl", 5, 1.1, 20);

  // right tail
  RooRealVar  cb_alpha_r("alphar", "alphar", 5, 0.000, 10);
  RooRealVar  cb_n_r("nr", "nr", 5, 1.1, 20);
  ...
  RooCBShape signal("signal", "signal", mass, mean, sigma, cb_alpha_l, cb_n_l );

  // background model
  RooRealVar expConstOne("expConstOne", "expConstOne", -0.6, -5, -1e-4);
  RooExponential background("background", "background", mass, expConstOne);

  // magnitudes
  RooRealVar  f_signal("f_signal", "f_signal", 0.5, 0, 1);
...

  {
    // information
    mainPad->cd();
    auto pave = new TPaveText(0.14,0.56,0.52,0.86, "NDC");
    pave->SetFillColor(0);
    pave->SetFillStyle(0);
