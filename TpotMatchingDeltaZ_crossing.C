...
//_______________________________________________________________
void TpotMatchingDeltaZ_crossing()
{
  ...
  const TString tag = "_CombinedDataReconstruction-new";
  TString postfix;
  TString run_label;
  ...
  const TString pdfFile = match_silicons ?
    Form( "Figures/TpotMatchingDeltaZ_crossing_matched%s%s.pdf", tag.Data(),postfix.Data()):
    Form( "Figures/TpotMatchingDeltaZ_crossing%s%s.pdf", tag.Data(),postfix.Data());
  PdfDocument pdfDocument( pdfFile );

  const TString rootFilename = match_silicons ?
    Form( "Rootfiles/TpotMatchingDeltaZ_crossing_matched%s%s.root", tag.Data(),postfix.Data()):
    Form( "Rootfiles/TpotMatchingDeltaZ_crossing%s%s.root", tag.Data(),postfix.Data());
...
//     "&& (_tracks._chisquare/_tracks._ndf)<100"
  ...
  const TCut match_tpot_cut( full_string( "_tracks._nclusters_micromegas>0" ) );
...
//     const TString var = full_string( "_tracks._crossing" );
//     const auto cut = trk_cut && match_tpot_cut && TCut( full_string("_tracks._trk_state_z._layer==56 && _tracks._found_cluster_z._layer==56" ));

//     const TString var = full_string( "_tracks._trk_state_z._z-_tracks._best_cluster_z._z");
//     const auto cut = TCut( full_string("_tracks._trk_state_z._layer==56 && _tracks._best_cluster_z._layer==56" )) && trk_cut;
    ...
    TString hname = "h_crossing";
    auto h = new TH1F( hname, "", 100, -100, 500 );
    Utils::TreeToHisto(tree, h->GetName(), var, cut, false);
    h->SetTitle("");
    h->GetXaxis()->SetTitle( "crossing" );
    h->SetStats(0);
...
//     const auto cut = TCut( full_string("_tracks._trk_state_z._layer==56 && _tracks._best_cluster_z._layer==56" )) && trk_cut;
    ...
    const TString var2d = Form(full_string("%s:_tracks._crossing"), var.Data() );

    TString hname = "h_dz";
    auto h = new TH2F( hname, "", 100, -100, 400, 100, -max_dz, max_dz);
