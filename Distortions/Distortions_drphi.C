...
//_________________________________________________________________________
TString Distortions_drphi()
{
  std::cout << "Distortions_drphi - bins: " << m_totalbins << std::endl;
  ...
  if( use_truth_information ) rootfilename = Form( "Rootfiles/Distortions_drphi%s_truth%s.root", tag.Data(), subtag.Data() );
  else if( use_micromegas ) rootfilename = Form( "Rootfiles/Distortions_drphi%s_mm%s.root", tag.Data(), subtag.Data() );
  else rootfilename = Form( "Rootfiles/Distortions_drphi%s_all%s.root", tag.Data(), subtag.Data() );
  RootFile rootFile( rootfilename );

  // log file
  TString logfilename;
  if( use_truth_information ) logfilename = Form( "Distortions_drphi%s_truth%s.log", tag.Data(), subtag.Data() );
  else if( use_micromegas ) logfilename = Form( "Distortions_drphi%s_mm%s.log", tag.Data(), subtag.Data() );
  else logfilename = Form( "Distortions_drphi%s_all%s.log", tag.Data(), subtag.Data() );

  std::cout << "Distortions_drphi - inputfile: " << inputFile << std::endl;
  std::cout << "Distortions_drphi - rootfile: " << rootfilename << std::endl;
  std::cout << "Distortions_drphi - logfile: " << logfilename << std::endl;
  ...
  const int offset = 0;
  std::cout << "Distortions_drphi - entries: " << entries << std::endl;
  for( int i = 0; i < entries; ++i )
  {
    tree->GetEntry(i + offset);
    if( !(i%100) )
      std::cout << "Distortions_drphi - entry: " << i << std::endl;
    ...
    for( const auto& track:container->tracks() )
    {
      ...
      for( const auto& cluster:track._clusters )
      {
        ...
        lhs[icell](0,0) += 1./erp;
        lhs[icell](0,1) += talpha/erp;

        lhs[icell](1,0) += talpha/erp;
        lhs[icell](1,1) += square(talpha)/erp;

        rhs[icell](0,0) += drp/erp;
        rhs[icell](1,0) += talpha*drp/erp;

        if( save_histograms )
        {
          residuals_drphi[icell]->Fill( drp );
          if( residuals_2d_drphi[icell] )
          { residuals_2d_drphi[icell]->Fill( talpha, drp ); }
        }
  ...
  // print statistics
  std::cout << "Distortions_drphi - entries: " << entries << std::endl;
  std::cout << "Distortions_drphi - track statistics total: " << total_tracks << " accepted: " << accepted_tracks << " fraction: " << 100.*accepted_tracks/total_tracks << "%" << std::endl;
  std::cout << "Distortions_drphi - cluster statistics total: " << total_clusters << " accepted: " << accepted_clusters << " fraction: " << 100.*accepted_clusters/total_clusters << "%" << std::endl;
