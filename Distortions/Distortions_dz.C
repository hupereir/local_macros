...
//_________________________________________________________________________
TString Distortions_dz()
{
  std::cout << "Distortions_dz - bins: " << m_totalbins << std::endl;
  ...
  if( save_histograms )
  {
      ...
      for( int iz = 0; iz < m_zbins; ++iz )
    {
      ...
      if( iphi+1 == phibin_rec && iz+1 == zbin_rec )
      {
        ...
        residuals_2d_dz[icell].reset( new TH2F( hname, hname, 100, -0.5, 0.5, 100, -max_residual_dz, +max_residual_dz ) );
        ...
        residuals_2d_dz[icell]->GetXaxis()->SetTitle( "tan#beta" );
...
//   const TString subtag = "-coarse";
  ...
  if( use_truth_information ) rootfilename = Form( "Rootfiles/Distortions_dz%s_truth%s.root", tag.Data(), subtag.Data() );
  else if( use_micromegas ) rootfilename = Form( "Rootfiles/Distortions_dz%s_mm%s.root", tag.Data(), subtag.Data() );
  else rootfilename = Form( "Rootfiles/Distortions_dz%s_all%s.root", tag.Data(), subtag.Data() );
  RootFile rootFile( rootfilename );

  // log file
  TString logfilename;
  if( use_truth_information ) logfilename = Form( "Distortions_dz%s_truth%s.log", tag.Data(), subtag.Data() );
  else if( use_micromegas ) logfilename = Form( "Distortions_dz%s_mm%s.log", tag.Data(), subtag.Data() );
  else logfilename = Form( "Distortions_dz%s_all%s.log", tag.Data(), subtag.Data() );

  std::cout << "Distortions_dz - inputfile: " << inputFile << std::endl;
  std::cout << "Distortions_dz - rootfile: " << rootfilename << std::endl;
  std::cout << "Distortions_dz - logfile: " << logfilename << std::endl;
  ...
  const int offset = 0;
  std::cout << "Distortions_dz - entries: " << entries << std::endl;
  for( int i = 0; i < entries; ++i )
  {
    tree->GetEntry(i + offset);
    if( !(i%100) )
      std::cout << "Distortions_dz - entry: " << i << std::endl;
    ...
    for( const auto& track:container->tracks() )
    {
      ...
      for( const auto& cluster:track._clusters )
      {
        ...
        lhs[icell](0, 0) += 1./ez;
        lhs[icell](0, 1) += tbeta/ez;

        lhs[icell](1,0) += tbeta/ez;
        lhs[icell](1,1) += square(tbeta)/ez;

        rhs[icell](0,0) += dz/ez;
        rhs[icell](1,0) += tbeta*dz/ez;

        if( save_histograms )
        {
          residuals_dz[icell]->Fill( dz );
          if( residuals_2d_dz[icell] )
          { residuals_2d_dz[icell]->Fill( tbeta, dz ); }
        }
    ...
    for( int iz = 0; iz < m_zbins; ++iz )
  {
    ...
    hz_rec->SetBinContent( iphi+1, ir+1, iz+1, result(0) );
    hz_rec->SetBinError( iphi+1, ir+1, iz+1, std::sqrt( cov(0,0) ) );

    hr_rec->SetBinContent( iphi+1, ir+1, iz+1, result(1) );
    hr_rec->SetBinError( iphi+1, ir+1, iz+1, std::sqrt( cov(1,1) ) );

    out << "Distortions - inverting bin " << iz << ", " << ir << ", " << iphi << std::endl;
    out << "Distortions - entries: " << binentries[icell] << std::endl;
    out << "Distortions - lhs: \n" << lhs[icell] << std::endl;
    out << "Distortions - rhs: \n" << rhs[icell] << std::endl;
    out << "Distortions - dz: " << result(0) << " +/- " << std::sqrt( cov(0,0) ) << std::endl;
    out << "Distortions - dr: " << result(1) << " +/- " << std::sqrt( cov(1,1) ) << std::endl;
    out << std::endl;
  }
  ...
  // print statistics
  std::cout << "Distortions_dz - entries: " << entries << std::endl;
  std::cout << "Distortions_dz - track statistics total: " << total_tracks << " accepted: " << accepted_tracks << " fraction: " << 100.*accepted_tracks/total_tracks << "%" << std::endl;
  std::cout << "Distortions_dz - cluster statistics total: " << total_clusters << " accepted: " << accepted_clusters << " fraction: " << 100.*accepted_clusters/total_clusters << "%" << std::endl;
