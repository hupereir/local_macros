  ...
  for( int i = 0; i < entries; ++i )
  {
    ...
    for( const auto& track:container->tracks() )
    {
      ...
      for( const auto& cluster:track._clusters )
      {
        ...
        matrix_container.add_to_lhs(index, 0, 0, r/erp );
        ...
        matrix_container.add_to_lhs(index, 2, 0, r*talpha/erp );
...
void MatrixInversion( const char* inputfile, const char* outputfilename )
{
  ...
  matrix_inversion.save_distortion_corrections( outputfilename );
}
...
void MatrixInversion( TString tag = TString() )
{
  if( tag.IsNull() )
  {
    ...
    // tag = "_flat_genfit_truth_notpc_nodistortion";
    // tag = "_flat_genfit_truth_notpc_distorted";
  }

  const TString inputfile = Form( "DST/CONDOR%s/TpcSpaceChargeMatrices_offline_test%s_*.root", tag.Data(), tag.Data() );
