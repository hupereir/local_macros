...
TH3* create_mask( TH3* h )
{
  auto hmask = static_cast<TH3*>( h->Clone( "mask" ) );
  hmask->Reset();
...
//_________________________________________
TH3* extrapolate_phi( TH3* h, TH2* h_cm, TH3* mask )
{

  // copy histogram
  auto h_copy = static_cast<TH3*>(h->Clone(TString(h->GetName())+"_extrap"));
  h_copy->Reset();
...
//_________________________________________
TH3* extrapolate_phi2( TH3* h, TH3* mask )
{

  // copy histogram
  auto h_copy = static_cast<TH3*>(h->Clone(TString(h->GetName())+"_extrap2"));
  h_copy->Reset();
    ...
    for( int iz = 0; iz < h->GetNbinsZ(); ++iz )
  {
    ...
    for( int ip = 0; ip < h->GetNbinsX(); ++ip )
    {
      ...
      if( ip_max == h->GetNbinsX() ) continue;

      // do the interpolation
      const double phi_min =  h->GetXaxis()->GetBinCenter(ip_min+1);
      const double phi_max =  h->GetXaxis()->GetBinCenter(ip_max+1);
      const double phi = h->GetXaxis()->GetBinCenter(ip+1);

      const double distortion_min = h->GetBinContent( ip_min+1, ir+1, iz+1 );
      const double distortion_max = h->GetBinContent( ip_max+1, ir+1, iz+1 );
      const double distortion = distortion_min + (phi-phi_min)*(distortion_max - distortion_min)/(phi_max-phi_min);

      h_copy->SetBinContent( ip+1, ir+1, iz+1, distortion );
    }
  }
...
//_________________________________________
TH3* extrapolate_z( TH3* h, TH3* mask )
{

  // copy histogram
  auto h_copy = static_cast<TH3*>(h->Clone(TString(h->GetName())+"_extrapz"));
  h_copy->Reset();
    ...
    for( int ip = 0; ip < h->GetNbinsX(); ++ip )
  {
    ...
    for( int iz = 0; iz < h->GetNbinsZ(); ++iz )
    {
      ...
      if( iz_max == h->GetNbinsZ() ) continue;

      // do the interpolation
      const double z_min =  h->GetZaxis()->GetBinCenter(iz_min+1);
      const double z_max =  h->GetZaxis()->GetBinCenter(iz_max+1);
      const double z = h->GetZaxis()->GetBinCenter(iz+1);

      const double distortion_min = h->GetBinContent( ip+1, ir+1, iz_min+1 );
      const double distortion_max = h->GetBinContent( ip+1, ir+1, iz_max+1 );
      const double distortion = distortion_min + (z-z_min)*(distortion_max - distortion_min)/(z_max-z_min);
...
void ExtrapolateDistortionCorrection()
{
  ...
  auto hentries_posz_extrapz = extrapolate_z( hentries_posz, hentries_posz );
  auto hentries_posz_extrap = extrapolate_phi( hentries_posz_extrapz, nullptr, hentries_posz_extrapz );
  ...
  for( const TString& hname: {"hIntDistortionR_posz", "hIntDistortionP_posz", "hIntDistortionZ_posz" } )
  {
    ...
    auto h_extrap = extrapolate_z( h, hentries_posz );
    h_extrap = extrapolate_phi( h_extrap, h_cm, hentries_posz_extrapz );
    h_extrap = extrapolate_phi2( h_extrap, hentries_posz_extrap );
  ...
  auto hentries_negz_extrapz = extrapolate_z( hentries_negz, hentries_negz );
  auto hentries_negz_extrap = extrapolate_phi( hentries_negz_extrapz, nullptr, hentries_negz_extrapz );
  for( const TString& hname: {"hIntDistortionR_negz", "hIntDistortionP_negz", "hIntDistortionZ_negz" } )
  {
    auto h = static_cast<TH3*>( in->Get(hname) );
    auto h_cm = static_cast<TH2*>( in_cm->Get(hname) );
    auto h_extrap = extrapolate_z( h, hentries_negz );
    h_extrap = extrapolate_phi( h_extrap, h_cm, hentries_negz_extrapz );
    h_extrap = extrapolate_phi2( h_extrap, hentries_negz_extrap );
