...
TString Distortions()
{
  ...
  const TString inputFile = Form( "DST/CONDOR%s/dst_reco%s_1??.root", tag.Data(), tag.Data() );

  // root file
  TString rootfilename;
  if( use_truth_information ) rootfilename = Form( "Rootfiles/Distortions_full%s_truth.root", tag.Data() );
  else if( use_micromegas ) rootfilename = Form( "Rootfiles/Distortions_full%s_mm.root", tag.Data() );
  else rootfilename = Form( "Rootfiles/Distortions_full%s_all.root", tag.Data() );
  ...
  matrix_inversion.set_outputfile( rootfilename.Data() );
  matrix_inversion.add( matrix_container );
