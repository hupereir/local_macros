...
TString Distortions()
{
  std::cout << "Distortions - bins: " << m_totalbins << std::endl;
...
//   const TString subtag = "-new";
  ...
  if( use_truth_information ) rootfilename = Form( "Rootfiles/Distortions_full%s_truth%s.root", tag.Data(), subtag.Data() );
  else if( use_micromegas ) rootfilename = Form( "Rootfiles/Distortions_full%s_mm%s.root", tag.Data(), subtag.Data() );
  else rootfilename = Form( "Rootfiles/Distortions_full%s_all%s.root", tag.Data(), subtag.Data() );
  RootFile rootFile( rootfilename );

  // log file
  TString logfilename;
  if( use_truth_information ) logfilename = Form( "Distortions%s_truth%s.log", tag.Data(), subtag.Data() );
  else if( use_micromegas ) logfilename = Form( "Distortions%s_mm%s.log", tag.Data(), subtag.Data() );
  else logfilename = Form( "Distortions%s_all%s.log", tag.Data(), subtag.Data() );

  std::cout << "Distortions - inputfile: " << inputFile << std::endl;
  std::cout << "Distortions - rootfile: " << rootfilename << std::endl;
  std::cout << "Distortions - logfile: " << logfilename << std::endl;
