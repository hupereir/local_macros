...
//_______________________________________________________________
void TpotMatchingCorrelation_best()
{
  ...
  // const TString tag = "_CombinedDataReconstruction_corrected";
  // const TString tag = "_CombinedDataReconstruction_zf_corrected-2";
  const TString tag = "_CombinedDataReconstruction-new2";
  TString postfix;
  TString run_label;
  ...
  const TString pdfFile = match_silicons ?
    Form( "Figures/TpotMatchingCorrelation_matched%s%s.pdf", tag.Data(),postfix.Data()):
    Form( "Figures/TpotMatchingCorrelation%s%s.pdf", tag.Data(),postfix.Data());
  ...
  if(true)
  {
    ...
    // phi layer
    {
      const auto var = full_string("_tracks._trk_state_phi._x_local:_tracks._clusters[]._x_local");
      const auto cut = TCut( full_string("_tracks._trk_state_phi._layer>0 && _tracks._clusters[]._layer==55")) && trk_cut;
      const TString var3d = Form( full_string("%s:_tracks._trk_state_phi._tile"),var.Data() );
    ...
    // z layer
    {
      const auto var = full_string("_tracks._trk_state_z._y_local:_tracks._clusters[]._y_local");
      const auto cut = TCut( full_string("_tracks._trk_state_z._layer>0 && _tracks._clusters[]._layer==56")) && trk_cut;
      const TString var3d = Form( full_string("%s:_tracks._trk_state_z._tile"),var.Data() );
  ...
  if(true)
  {
    // local coordinates, using best cluster only
    auto cv = new TCanvas( "cv3", "cv3", 800, 800 );
    cv->Divide(4, 4);

    // phi layer
    {
      const auto var = full_string("_tracks._trk_state_phi._x_local:_tracks._best_cluster_phi._x_local");
      const auto cut = TCut( full_string("_tracks._trk_state_phi._layer>0 && _tracks._best_cluster_phi._layer==55")) && trk_cut;
      const TString var3d = Form( full_string("%s:_tracks._trk_state_phi._tile"),var.Data() );
    ...
    // z layer
    {
      const auto var = full_string("_tracks._trk_state_z._y_local:_tracks._best_cluster_z._y_local");
      const auto cut = TCut( full_string("_tracks._trk_state_z._layer>0 && _tracks._best_cluster_z._layer==56")) && trk_cut;
      const TString var3d = Form( full_string("%s:_tracks._trk_state_z._tile"),var.Data() );
  ...
  {
    // local coordinates
    auto cv = new TCanvas( "cv6", "cv6", 800, 800 );
  ...
  {
    // local coordinates
    auto cv = new TCanvas( "cv7", "cv7", 800, 800 );
