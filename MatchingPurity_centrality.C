...
//____________________________________________________________________________
TString MatchingPurity_centrality()
{
  set_style( false );
...
//   const TString inputFile = Form( "DST/CONDOR%s/dst_eval*.root", tag.Data() );
  ...
  const TString pdfFile = use_strict ?
    Form( "Figures/MatchingPurity_centrality%s_strict.pdf", tag.Data() ):
    Form( "Figures/MatchingPurity_centrality%s.pdf", tag.Data() );

  const TString rootFilename = use_strict ?
    Form( "Rootfiles/MatchingPurity_centrality%s_strict.root", tag.Data() ):
    Form( "Rootfiles/MatchingPurity_centrality%s.root", tag.Data() );

  std::cout << "MatchingPurity_centrality - inputFile: " << inputFile << std::endl;
  std::cout << "MatchingPurity_centrality - pdfFile: " << pdfFile << std::endl;
  std::cout << "MatchingPurity_centrality - rootFilename: " << rootFilename << std::endl;
...
//   constexpr std::array<int, 10> color = { kBlue, kCyan+2, kGreen+2, kOrange+1, kRed, kBlue, kCyan+2, kGreen+2, kOrange+1, kRed };
  ...
  for( int i = 0; i < 2; ++i )
  {
    ...
    std::cout << "MatchingPurity_centrality - layer: " << layer << std::endl;
    ...
    // plot
    std::cout << "MatchingPurity_centrality - plotting" << std::endl;
    auto cv( new TCanvas( "cv", "cv", 800, 800 ) );
    for( int icent = 0; icent < ncentbins; ++icent )
    {

      auto heff = heff_array[icent];
      if( !heff ) { std::cout << "MatchingPurity_centrality - invalid histogram" << std::endl; }
      heff->SetMarkerStyle( 20 );
      heff->SetMarkerColor( color[icent] );
      heff->SetLineColor( color[icent] );
