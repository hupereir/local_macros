...
TString ClusterSizeRPhi()
{

  set_style( false );

  // input files
  const TString tag = "_acts_full_notpc_nodistortion-old";
  const TString inputFile = Form( "DST/CONDOR_realistic_micromegas/dst_reco%s/dst_reco*_?.root", tag.Data() );

  const TString pdfFile = Form( "Figures/ClusterSizeRPhi%s.pdf", tag.Data() );

  std::cout << "ClusterSizeRPhi - inputFile: " << inputFile << std::endl;
  std::cout << "ClusterSizeRPhi - pdfFile: " << pdfFile << std::endl;
  ...
  // variable names
  const TString var( "_clusters._phi_size" );
  ...
  for( int idet = 0; idet < nDetectors; ++idet )
  {
    ...
    const TString hname( Form( "clusterSizeRphi_%i", idet ) );
    ...
    for( int ilayer = 0; ilayer < nLayers[idet]; ++ilayer )
    {
      ...
      h->SetLineColor( 1 );
      h->SetMarkerColor( 1 );
      h->GetXaxis()->SetTitle( "csize_{r#phi}" );
