...
//____________________________________________________________________________
TString PullsZ_cluster_mvtx( TString tag = TString() )
{
  ...
  #if true
  const TString pdfFile = Form( "Figures/PullsZ_cluster%s_mvtx_0.pdf", tag.Data() );
  constexpr int n_cluster_cut = 3;
  std::array<TCut, n_cluster_cut> cluster_cuts =
  {{
    "_clusters._z_size == 1 && _clusters._phi_size == 1",
    "_clusters._z_size == 2",
    "_clusters._z_size == 3"
  }};
  ...
  #else
  const TString pdfFile = Form( "Figures/PullsZ_cluster%s_mvtx_1.pdf", tag.Data() );
  constexpr int n_cluster_cut = 4;
  std::array<TCut, n_cluster_cut> cluster_cuts =
  {{
    "_clusters._z_size == 2 && _clusters._phi_size == 2 ",
    "_clusters._z_size == 2 && _clusters._phi_size == 3 ",
    "_clusters._z_size == 3 && _clusters._phi_size == 2 ",
    "_clusters._z_size == 3 && _clusters._phi_size == 3 "
  }};

  std::array<TString, n_cluster_cut> cut_label =
  {{
    "csize_{z} = 2, csize_{#phi} = 2",
    "csize_{z} = 2, csize_{#phi} = 3",
    "csize_{z} = 3, csize_{#phi} = 2",
    "csize_{z} = 3, csize_{#phi} = 3"
  }};
  #endif
  ...
  // scale factor
  static const float length = 0.002924;
  static const float scale = length/std::sqrt(12.);

  // variable names
  const TString var = Form( "(_clusters._z - _clusters._truth_z)/%f", scale );
  ...
  for( int idet = 0; idet < nDetectors; ++idet )
  {
    ...
    for( int icut = 0; icut < n_cluster_cut; ++icut )
    {
      ...
      const TString hname( Form( "PullsZ_%i_%i", idet, icut ) );
      ...
      for( int ilayer = 0; ilayer < nLayers[idet]; ++ilayer )
      {
        ...
        h->SetLineColor( 1 );
        h->SetMarkerColor( 1 );
        // h->GetXaxis()->SetTitle( "#Deltaz_{clus-truth}.#sqrt{12}/length" );
        h->GetXaxis()->SetTitle( "#Deltaz_{clus-truth}/#sigma_{z}" );
