...
void Lambda0Mass_Roofit()
{
  ...
  const auto pdffilename = drawPulls ?
    Form( "Figures/Lambda0Mass_Roofit_Pulls%s.pdf", runPostfix.Data() ):
    Form( "Figures/Lambda0Mass_Roofit%s.pdf", runPostfix.Data() );
  ...
  std::cout << "Lambda0Mass_Roofit - done with copy" << std::endl;

  TTree* clone = copy->CloneTree(-1);
  std::cout << "Lambda0Mass_Roofit - done with clone" << std::endl;

  // ROO variable
  const TString var = "Lambda0_mass";
  const double mass_min = 1.08;
  const double mass_max = 1.19;
  const int nbins = 110;
  ...
  // signal model
  RooRealVar  mean("mean", "mean", 1.115, 1.08, 1.19);
  RooRealVar  sigma("sigma", "sigma", 0.004, 0.000, 0.01);
  ...
  // background model
  RooRealVar mean_bw("mean_bw", "mean_bw", 1.18, 1.16, 1.5);
  RooRealVar sigma_bw("sigma_bw", "sigma_bw", 0.1, 0, 0.3);
  RooBreitWigner background("background", "background", mass, mean_bw, sigma_bw);
...

  {
    ...
    mainPad->cd();
    frame->SetTitle("");
    frame->GetXaxis()->SetTitle( "m(p^{#pm}#pi^{#mp}) [GeV]");
  ...
  if( drawPulls )
  {
...

    {
      ...
      frame_pulls->GetXaxis()->SetTitle("m(p^{#pm}#pi^{#mp}) [GeV]");
...

  {
    ...
    legend->AddEntry(frame->findObject("h_Lambda0_mass"),"Data","PE2");
    legend->AddEntry(frame->findObject("model_Norm[Lambda0_mass]"),"Fit","L");
    legend->AddEntry(frame->findObject("model_Norm[Lambda0_mass]_Comp[signal,background]"),"#Lambda^{0}#rightarrowp^{#pm}#pi^{#mp}","f");
    legend->AddEntry(frame->findObject("model_Norm[Lambda0_mass]_Comp[background]"),"Comb. Bkg.","f");
