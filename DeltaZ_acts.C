...
TString DeltaZ_acts()
{
  ...
  set_style( false );
  const TString tag = "_truth_realistic";
  const TString inputFile = Form( "DST/TpcResiduals%s.root", tag.Data() );

  const TString pdfFile = Form( "Figures/DeltaZ_acts%s.pdf", tag.Data() );
  const TString rootFile  = Form( "Rootfiles/DeltaZ_acts%s.root", tag.Data() );
  ...
  auto h2d = static_cast<TH2*>(fileManager.GetHistogram( "deltaz_layer" ));
