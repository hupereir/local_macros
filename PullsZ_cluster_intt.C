...
//____________________________________________________________________________
TString PullsZ_cluster_intt( TString tag = TString() )
{
...
//   // input files
  ...
  const TString inputFile = Form( "DST/CONDOR%s/dst_eval%s*_1*.root", tag.Data(), tag.Data() );

//   if( tag.IsNull() ) tag = "_1k_realistic_full_nominal_new2";
//   const TString inputFile = Form( "DST/dst_eval%s.root", tag.Data() );

  const TString pdfFile = Form( "Figures/PullsZ_cluster%s_intt.pdf", tag.Data() );
  constexpr int n_cluster_cut = 1;
  ...
  for( int idet = 0; idet < nDetectors; ++idet )
  {
    ...
    for( int icut = 0; icut < n_cluster_cut; ++icut )
    {
      ...
      for( int ilayer = 0; ilayer < nLayers[idet]; ++ilayer )
      {
        ...
        h->SetLineColor( 1 );
        h->SetMarkerColor( 1 );
        h->GetXaxis()->SetTitle( "#Deltaz_{clus-truth}.#sqrt{12}/pitch" );
