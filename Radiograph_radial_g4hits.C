...
//   const TString inputFile = Form( "DST/dst_reco%s.root", tag.Data() );
  ...
  // pdf output
  const TString pdfFile = Form( "Figures/Radiograph_radial_g4hits%s.pdf", tag.Data() );
  ...
  if( true )
  {
    ...
    const TString var( "_g4hits._r:get_phi(_g4hits._phi)" );
