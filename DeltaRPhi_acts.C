...
TString DeltaRPhi_acts( TString tag = TString() )
{

  set_style( false );

  // if( tag.IsNull() ) tag = "_acts_full_notpc_nodistortion";
  if( tag.IsNull() ) tag = "_acts_full_notpc_nodistortion-new";
  // if( tag.IsNull() ) tag = "_acts_full_notpc_nodistortion";
  // const TString inputFile = Form( "DST/CONDOR_flat_micromegas/dst_reco%s/TpcResiduals*.root", tag.Data() );
  const TString inputFile = Form( "DST/CONDOR_realistic_micromegas/dst_reco%s/TpcResiduals*.root", tag.Data() );

  const TString pdfFile = Form( "Figures/DeltaRPhi_acts%s.pdf", tag.Data() );
  const TString rootFile  = Form( "Rootfiles/DeltaRPhi_acts%s.root", tag.Data() );
  ...
  auto h2d = static_cast<TH2*>(fileManager.GetHistogram( "deltarphi_layer" ));
  ...
  for( int idet = 0; idet < nDetectors; ++idet )
  {
    ...
    for( int ilayer = 0; ilayer < nLayers[idet]; ++ilayer )
    {
      ...
      // const int binIndex = layerIndex - firstLayer_tpc + 1;
