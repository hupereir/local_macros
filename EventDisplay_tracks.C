...
void EventDisplay_tracks()
{
  ...
  const TString pdffilename = Form( "Figures/EventDisplay_tracks%s.pdf", tag.Data() );
  ...
  for( int i = 0; i < entries; ++i )
  {
    ...
    std::cout << "EventDisplay - entry: " << i << " tracks: " << tracks.size() << std::endl;
    ...
    for( const auto& track:tracks )
    {
      ...
      for( const auto& cluster:track._calo_clusters )
      {
        marker->DrawMarker( cluster._x, cluster._y );
      }
