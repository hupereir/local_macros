...
TString DrawMatchingPurity_centrality()
{
  set_style( false );
...
//   constexpr std::array<int, 10> color = { kBlue, kCyan+2, kGreen+2, kOrange+1, kRed, kBlue, kCyan+2, kGreen+2, kOrange+1, kRed };
  ...
  const TString inputFile = Form( "Rootfiles/MatchingPurity_centrality%s.root", tag.Data() );
  const TString pdfFile = Form( "Figures/MatchingPurity_centrality%s.pdf", tag.Data() );
  ...
  for( int i = 0; i < 2; ++i )
  {
    ...
    std::cout << "DrawMatchingPurity_centrality - layer: " << layer << std::endl;
