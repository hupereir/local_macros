...
TString DeltaZMomentum_truth( TString tag = TString() )
{
  ...
  const int layer = 7;
  const auto maxResidual = 0.4;

  // pdf output
  // if( tag.IsNull() ) tag = "_5k_truth_notpc_noouter" ;
  if( tag.IsNull() ) tag = "_5k_truth_low_momentum_notpc_noouter" ;
  const TString inputFile = Form( "DST_afs/dst_eval%s.root", tag.Data() );
  const TString pdfFile = Form( "Figures/DeltaZMomentum_truth%s_%i.pdf", tag.Data(), layer );
...
//   const int nptbins = 15;
  ...
  auto h2d( new TH2F( hname, "", nptbins, ptmin, ptmax, 50, -maxResidual, maxResidual ) );
