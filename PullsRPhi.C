...
float quadratic_sum( const float& x1, const float& x2 )
...
//   // input files
//   const TString tag = "_flat_acts_full_no_distortion";
...
//   const TString tag = "_acts_truth_notpc_nodistortion";
  ...
  const TString pdfFile = Form( "Figures/PullsRPhi%s.pdf", tag.Data() );
  const TString rootFile  = Form( "Rootfiles/PullsRPhi%s.root", tag.Data() );

  std::cout << "PullsRPhi - inputFile: " << inputFile << std::endl;
  std::cout << "PullsRPhi - pdfFile: " << pdfFile << std::endl;
  ...
  // variable names
  // const TString var( "delta_phi(_tracks._clusters._trk_phi - _tracks._clusters._phi)/quadratic_sum(_tracks._clusters._phi_error, _tracks._clusters._trk_phi_error)" );
  
  const TString var( "delta_phi(_tracks._clusters._trk_phi - _tracks._clusters._phi)/"
    "sqrt( _tracks._clusters._trk_phi_error*_tracks._clusters._trk_phi_error"
    "+ _tracks._clusters._phi_error*_tracks._clusters._phi_error)" );
  
  // const TString var( "delta_phi(_tracks._clusters._trk_phi - _tracks._clusters._phi)/sqrt(_tracks._clusters._trk_phi_error*_tracks._clusters._trk_phi_error)" );
  const TString var2d = Form( "%s:_tracks._clusters._layer", var.Data() );
  const TCut cluster_cut( "_tracks._clusters._phi_error > 0 && _tracks._clusters._trk_phi_error > 0" );
  ...
  for( int idet = 0; idet < nDetectors; ++idet )
  {
    ...
    for( int ilayer = 0; ilayer < nLayers[idet]; ++ilayer )
    {
      ...
      h->SetLineColor( 1 );
      h->SetMarkerColor( 1 );
      h->GetXaxis()->SetTitle( "#Delta#phi_{track-clus}/#sigma#phi" );
      ...
      if( entries )
      {
        if( do_fit )
        {
          ...
          if( result._valid )
          {
            ...
            std::cout << "PullsRPhi - skipping layer " << layerIndex << " (failed fit)" << std::endl;
  ...
  // TGraph
  {
    ...
    h->GetYaxis()->SetTitle( "#sigma_{r.#Delta#phi} (track-cluster)" );
