...
void PhiMass_Roofit()
{
  ...
  const bool use_all_momentum = false;
  const TString tag = use_all_momentum ? "_all_momentum":"_low_momentum";

  const auto pdffilename = drawPulls ?
    Form( "Figures/PhiMass_Roofit_Pulls%s%s.pdf", tag.Data(), runPostfix.Data() ):
    Form( "Figures/PhiMass_Roofit%s%s.pdf", tag.Data(), runPostfix.Data() );
  ...
  // cut
  TCut cut= mvtx_states_cut && tpc_states_cut&&track_chi2_cut
    &&track_pt_cut
    &&track_dca_cut
    &&decay_length_cut;

  if( !use_all_momentum ) { cut = cut&&track_p_cut; }

  auto copy = tree->CopyTree(cut);
  std::cout << "PhiMass_Roofit - done with copy" << std::endl;

  auto clone = copy;
  // auto clone = copy->CloneTree(-1);
  std::cout << "PhiMass_Roofit - done with clone" << std::endl;

  // ROO variable
  const TString var = "phi_mass";
  const double mass_min = 0.99;
  ...
  #ifdef USE_HISTOGRAM
  auto h = new TH1F( "mass","", nbins, mass_min, mass_max );
  Utils::TreeToHisto( clone, h->GetName(), var, TCut(), false );
  RooDataHist dataSet(var,"data",mass,h);
  #else
  RooDataSet dataSet(var, "data", mass, RooFit::Import(*clone));
  #endif

  // signal model
  RooRealVar  mean("mean", "mean", 1.02, 0.98, 1.1);
  RooRealVar  sigma("sigma", "sigma", 0.003, 0.000, 0.01);
  ...
  RooRealVar p1("p1","coeff #1", -15, -100., 100.);
  RooRealVar p2("p2","coeff #2", -0.4, -100., 100.);
  RooRealVar p3("p3","coeff #3", 15, -100., 100.);
  RooPolynomial poly("poly","background", mass, RooArgList(p1, p2, p3));

  // argus background
  const double mass_min_bg = 0.987;
  RooRealVar k("k","k",-10,-100,-0.1);
  RooRealVar edp("edp","edp",mass_min);
  RooFormulaVar shifted("shifted", "shifted", Form("-%s+2*%f",var.Data(),mass_min), RooArgSet(mass));
  RooArgusBG argus("argus","background", shifted, edp, k);

  RooRealVar fBkg("fBkg", "Fraction of Argus", 0.5, 0.0, 1.0);
  RooAddPdf background("background", "Composite Background", RooArgList(argus, poly), RooArgList(fBkg));

  // magnitudes
  RooRealVar  f_signal("f_signal", "f_signal", 0.1, 0, 1);
...

  {
    ...
    mainPad->cd();
    frame->SetTitle("");
    frame->GetXaxis()->SetTitle( "m(K^{+}K^{-}) [GeV]");
  ...
  if( drawPulls )
  {
...

    {
      ...
      frame_pulls->GetXaxis()->SetTitle("m(K^{+}K^{-}) [GeV]");
...

  {
    // legend
    mainPad->cd();
    auto legend = use_all_momentum ?
      new TLegend(0.6,0.38,0.89,0.62):
      new TLegend(0.6,0.66,0.89,0.9);
    legend->AddEntry(frame->findObject("h_phi_mass"),"Data","PE2");
    legend->AddEntry(frame->findObject("model_Norm[phi_mass]"),"Fit","L");
    legend->AddEntry(frame->findObject("model_Norm[phi_mass]_Comp[signal,background]"),"#phi#rightarrowK^{+}K^{-}","f");
    legend->AddEntry(frame->findObject("model_Norm[phi_mass]_Comp[background]"),"Comb. Bkg.","f");
...

  {
    ...
    auto pave =
      use_all_momentum ?
      new TPaveText(0.58,0.02,0.96,0.32, "NDC"):
      new TPaveText(0.58,0.29,0.96,0.59, "NDC");
    pave->SetFillColor(0);
    pave->SetFillStyle(0);
