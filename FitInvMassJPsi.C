...
namespace
{
  ...
  const double mass_signal_min = 2.8;
  const double mass_signal_max = 3.3;
}
...
void FitInvMassJPsi()
{
  ...
  const TString tag = "_jpsi_np_pythia8_acts_full_no_distortion";
  const TString inputFile = Form( "Rootfiles/InvMassJPsi%s.root", tag.Data() );
  FileManager fileManager( inputFile );

  const TString pdfFile = Form( "Figures/InvMassJPsi%s.pdf", tag.Data() );
  ...
  std::cout << "FitInvMassJPsi - total: " << total << std::endl;
  std::cout << "FitInvMassJPsi - background: " << background << std::endl;
