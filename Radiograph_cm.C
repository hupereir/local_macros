...
//   const TString inputFile = Form( "DST/dst_reco%s.root", tag.Data() );
  ...
  // const TString tag = "_centralmembrane_distorted_scaled_x2-10ev";
  const TString tag = "_centralmembrane_distorted_full-10ev";
  const TString inputFile = Form( "DST/dst_eval%s.root", tag.Data() );

  std::cout << "Radiograph - inputFile: " << inputFile << std::endl;
  
  // pdf output
  const TString pdfFile = Form( "Figures/Radiograph_cm%s.pdf", tag.Data() );
  ...
  if( true )
  {
    ...
    const TString var( "_cm_clusters._y:_cm_clusters._x" );
    ...
    // cluster size vs side
    if( true )
    {
      auto cv = new TCanvas( "cv", "", 900, 450 );
      cv->Divide( nsidecuts, 1 );
      for( int iside = 0; iside < nsidecuts; ++iside )
      {
        auto hname =  Form( "hside_%i", iside );
        auto h = Utils::TreeToHisto( tree, hname, "_cm_clusters._nclusters", side_cuts[iside], kTRUE );
        h->SetTitle("");
        h->GetXaxis()->SetTitle( "cm cluster size" );
        cv->cd(iside+1);
        h->Draw();
     
        Draw::PutText( 0.15, 0.15, Form( "%s - entries: %.0f", side_labels[iside].Data(), h->GetEntries() ) );
