...
void EventDisplay_clusters_run3(int runnumber = 57674)
{

  const TString tag = "_CombinedDataClustering";
  const TString inputfile = Form( "DST/CONDOR%s/CLUSTERS-%08i-*-full.root", tag.Data(), runnumber );
  const TString pdffilename = Form( "Figures/EventDisplay_clusters%s-%08i.pdf", tag.Data(), runnumber );
...
//     std::cout << "EventDisplay - entry: " << i << std::endl;
    ...
    const auto  nclusters_micromegas = container->events()[0]._nclusters_micromegas;

    if( nclusters_micromegas <= 2 )
    { continue; }

    std::cout << "EventDisplay - entry: " << i
      << " clusters: " << clusters.size()
      << " tpot clusters: " << nclusters_micromegas
      << std::endl;
