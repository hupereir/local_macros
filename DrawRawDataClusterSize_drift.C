...
void DrawRawDataClusterSize_drift()
{
  ...
  for( const auto& [hv,runnumber]:run_map )
  {
      ...
      for( int itile = 0; itile <8; ++itile )
    {
      ...
      
      {
        ...
        const auto htitle = Form( "cluster size %s", detector_names[detid].c_str() );
        h->SetName( hname );
        h->SetTitle( htitle );
  ...
  // make plot
  const TString pdffilename = "Figures/RawDataClusterSize-drift.pdf";
  ...
  if( true )
  {
      ...
      for( int itile = 0; itile <8; ++itile )
    {
      ...
      // legend
      auto legend = new TLegend( 0.7, 0.16, 0.95, 0.41, "", "NDC" );
