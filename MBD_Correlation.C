...
//_________________________________________________
void MBD_Correlation(const int runnumber = 20446)
{
  set_style( false );
  ...
  const TString pdffilename = Form( "Figures/MBD_Correlation-%08i-0000.pdf", runnumber );
  const TString pngfilename = Form( "Figures/MBD_Correlation-%08i-0000.png", runnumber );
  const TString rootfilename = Form( "Rootfiles/MBD_Correlation-%08i-0000.root", runnumber );

  std::cout << "max clock: " << 0x10000 << std::endl;
...
//   // loop over entries using offset
//   size_t tpot_entry = 55627;
//   size_t mbd_entry = 56204;
  ...
  // const int max_entry = 8988;
  // const int max_entry = 9000;
  // const int max_entry = 50000;
  const int max_entry = 7e5;
  int tpot_entries = std::min<int>( max_entry, tpot_tree->GetEntries() );
  int mbd_entries = std::min<int>( max_entry, mbd_tree->GetEntries() );

  // correlation histogram
  auto h_correlation = new TH2F( "h_correlation", "", 320, 0, 2000, 320, 0, 320 );
  h_correlation->GetXaxis()->SetTitle( "MBD Q_{tot} [A.U.]" );
  h_correlation->GetYaxis()->SetTitle( "TPOT N_{clusters}" );
  rootfile.Add( h_correlation );
    
  // delta difference
  auto h_delta = new TH1F( "h_delta", "delta", 201, -100, 100 );
  bool first = true;
  rootfile.Add( h_delta );
  while( tpot_entry < tpot_entries && mbd_entry < mbd_entries )
  {
    ...
    while( tpot_time > mbd_time + 1000 && mbd_entry<mbd_entries )
    {
      ...
      /* 
       * check if this is not due to incorrect rollover count
    ...
    if( !corrected ) 
    {
      h_delta->Fill( int64_t(mbd_time)-tpot_time ); 
    }
  ...
  if( true )
  {
    auto cv = new TCanvas( "cv", "cv", 1200, 900 );
  ...
  if( true )
  {
    auto cv = new TCanvas( "cv1", "cv1", 1200, 900 );
    h_delta->Draw();
    gPad->SetLogy(true);
    pdfDocument.Add(cv);
  } 
