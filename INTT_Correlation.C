...
//_________________________________________________
void INTT_Correlation( const int runnumber = 20445 )
{
  set_style( false );
  ...
  const TString pdffilename = Form( "Figures/INTT_Correlation-%08i-0000_intt%i.pdf", runnumber, intt_id );
  const TString pngfilename = Form( "Figures/INTT_Correlation-%08i-0000_intt%i.png", runnumber, intt_id );
  ...
  const int max_entry = 1e8;
  int intt_entries = std::min<int>( max_entry, intt_tree->GetEntries() );
  int tpot_entries = std::min<int>( max_entry, tpot_tree->GetEntries() );

  // correlation histogram
  auto h_correlation = new TH2F( "h_correlation", "", 320, 0, 320, 320, 0, 3000 );
  ...
  while( tpot_entry < tpot_entries && intt_entry < intt_entries )
  {
    ...
    if( !corrected ) 
    { 
      h_correlation->Fill( tpot_container->n_clusters, intt_event->fNhits );
    }
